...
export const Dock: React.FC<DockProps> = ({ openApp, runningApps }) => {
  ...
  return (
    <footer className="fixed bottom-1 sm:bottom-2 left-1/2 -translate-x-1/2 z-40">
      <div className="flex items-end h-16 sm:h-20 p-1 sm:p-2 space-x-1 sm:space-x-2 bg-white/20 backdrop-blur-xl rounded-2xl shadow-lg border border-white/30 overflow-x-auto">
        {APPS.map(app => (
          <div key={app.name} className="relative flex flex-col items-center group">
             <div className="absolute bottom-full mb-2 w-max px-2 py-1 bg-black/70 text-white text-xs rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
              {app.name}
            </div>
            <button
